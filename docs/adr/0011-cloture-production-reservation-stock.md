...
production). Placé en **2ᵉ onglet de la section « Stock »** (Inventaire / Puis-je produire ?),
...
L'action **« Clôturer / Produire (nb réel de cartes) »** est offerte dans l'onglet
**« Puis-je produire ? »** (on vérifie la faisabilité puis on clôture au même endroit),
avec la liste des lots et leur annulation. L'endpoint restant namespacé sous
`/machines/...`, un déclencheur bouton dans Machine PnP (après `validate-order`) est un
ajout ultérieur trivial.
